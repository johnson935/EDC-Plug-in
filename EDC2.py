        ...
        #                        direction2 = np.dot(Rz,plyaxis2.direction)
            ...
            #            
                ...
                if  totalU not in U.keys():  
                    ...
                    U[totalU].append(p.features.keys()[-2])
                    pos[totalU].append(z)
                print('Total energy dissipated from failure: {0} at Point: {1}'.format(float(totalU),pt2))
                plotU.append(totalU)
        critU = min(U.keys())
        planes = U[critU]   
        print('Critical energy is: {0}kJ at the following planes: {1}, at Point {2}'.format(critU,planes,pt2))
        t2 = time.time()
        print('Run time: {0}'.format(t2-t1))
